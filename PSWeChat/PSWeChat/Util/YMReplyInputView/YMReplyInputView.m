...
- (void) composeView
{
	...
	textView = [[UITextView alloc] initWithFrame:CGRectMake(70.0f, topGap, 185 + K_SCREEN_WIDTH - 320, 0)];
...
- (void)layoutSubviews
{
  ...
  //最上面的一条细线
    UILabel *dividLbl = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, K_SCREEN_WIDTH, 1)];
    ...
    textView.frame = CGRectMake(5, textView.frame.origin.y, K_SCREEN_WIDTH - 10 - 65, textView.frame.size.height);
    ...
    lblPlaceholder.backgroundColor = [UIColor clearColor];
    sendButton.frame = CGRectMake(K_SCREEN_WIDTH - 10 - 55,textView.frame.origin.y, 55, 27);
}
...
- (id) initWithFrame:(CGRect)frame andAboveView:(UIView *)bgView
{
    ...
    if (self)
    {
        tapView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, K_SCREEN_HEIGHT)];
