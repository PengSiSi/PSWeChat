...
- (void)viewDidLoad {
    ...
    if (self.displayActionButton) {
        if (_isShowDeleteRightNavBtn) {
            _actionButton = [UIBarButtonItem itemWithImage:@"lookover_delete_icon" higlightedImage:@"lookover_delete_icon" target:self action:@selector(actionButtonPressed:)];
        } else {
...
- (void)performLayout {
    ...
    // Right - Action
    if ((_actionButton && !(!hasItems && !self.navigationItem.rightBarButtonItem) ) && !_isShowDeleteRightNavBtn) {
...
- (void)showProgressHUDWithMessage:(NSString *)message {
    [CombancHUD showWithStatus:message];
...
- (void)showProgressHUDCompleteMessage:(NSString *)message {
    if (message) {
        [CombancHUD dismissWithDelay:1.5];
    } else {
        [CombancHUD dismiss];
    }
    self.navigationController.navigationBar.userInteractionEnabled = YES;
...
- (void)setDisplayNavArrows:(BOOL)displayNavArrows {
    _displayNavArrows = displayNavArrows;
}
...
- (void)setAlwaysShowControls:(BOOL)alwaysShowControls {
    _alwaysShowControls = alwaysShowControls;
}

- (void)setZoomPhotosToFill:(BOOL)zoomPhotosToFill {
    _zoomPhotosToFill = zoomPhotosToFill;
}
...
- (void)setStartOnGrid:(BOOL)startOnGrid {
    _startOnGrid = startOnGrid;
}
...
- (void)setAutoPlayOnAppear:(BOOL)autoPlayOnAppear {
    _autoPlayOnAppear = autoPlayOnAppear;
}

- (void)setIsShowDeleteRightNavBtn:(BOOL)isShowDeleteRightNavBtn {
    _isShowDeleteRightNavBtn = isShowDeleteRightNavBtn;
}
